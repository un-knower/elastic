	...
	*/
    ...
    InitDate.prototype = {
        ...
        setHtml:function(opts, self) {
            ...
            if(this.config.defaultTime){
            	var arr = this.config.defaultTime.split(':');
            	if(arr && arr.length){
            		date.setHours(arr[0]);
            ...
            if (opts.isTime) {
                var dhmsArr = jeDt.val(self) != "" || jeDt.text(self) != "" ? [ arrTime[3], arrTime[4], arrTime[5] ] :[ date.getHours(), date.getMinutes(), date.getSeconds() ];
    ...
    win.xsmDate = function(options){
    	var elements = QD(options.dateCell);
    	var result = [];
    	if(!elements || !elements.length){
    		return result;
    	}
    	for(var i=0;i<elements.length;i++){
    		options.dateCell = elements[i];
    		result.push(jeDate(options));
    	}
    	return result;
    };
})(window);